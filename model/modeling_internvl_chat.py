...
class InternVLChatModel(PreTrainedModel):
    ...
    def chat(self, tokenizer, pixel_values, question, generation_config, history=None, return_history=False,
        ...
        attention_mask = model_inputs['attention_mask'].cuda()
        generation_config['eos_token_id'] = eos_token_id
        # the default behavior is below line, if not set, we got warning log:
    ...
    def generate(
        ...
        if pixel_values is not None:
            ...
            # 151648
            # (Pdb++) selected
            # tensor([False, False, False,  ..., False, False, False], device='cuda:0') # most values are True
            # (Pdb++) selected.shape
            # torch.Size([3382])
            # (Pdb++) selected.sum()
            # tensor(3328, device='cuda:0')
            # (Pdb++) vit_embeds.shape
            # torch.Size([13, 256, 896])
            # (Pdb++) vit_embeds.reshape(-1, C).shape
            # torch.Size([3328, 896])
            # (Pdb++) input_embeds.shape
            # torch.Size([3382, 896])
        else:
            input_embeds = self.language_model.get_input_embeddings()(input_ids)
